...
<div class="flex flex-col md:flex-row min-h-screen bg-gray-100">
    ...
    <main class="flex-1 p-4">
        <div id="dashboard-content" class="content-section">
            <h1 class="text-3xl font-extrabold mb-8 text-gray-900">{{ __('general.Dashboard') }}</h1>
            
            <!-- Order Summary -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-white p-6 rounded-lg shadow-md">
                    <h3 class="text-lg font-semibold mb-2">{{ __('general.Recent Orders') }}</h3>
                    @if($orders->count() > 0)
                        <p class="text-gray-600">{{ __('general.You have :count recent orders.', ['count' => $orders->count()]) }}</p>
                        <a href="{{ route('buyer.dashboard.orders') }}" class="text-blue-500 hover:text-blue-700 mt-2 inline-block">{{ __('general.View all orders') }}</a>
                    @else
                        <p class="text-gray-600">{{ __('general.You have no recent orders.') }}</p>
                    @endif
                </div>
                
                <!-- Wishlist Summary -->
                <div class="bg-white p-6 rounded-lg shadow-md">
                    <h3 class="text-lg font-semibold mb-2">{{ __('general.Wishlist') }}</h3>
                    @if($wishlistItems && $wishlistItems->count() > 0)
                        <p class="text-gray-600">{{ __('general.You have :count items in your wishlist.', ['count' => $wishlistItems->count()]) }}</p>
                        <a href="{{ route('wishlist.index') }}" class="text-blue-500 hover:text-blue-700 mt-2 inline-block">{{ __('general.View wishlist') }}</a>
                    @else
                        <p class="text-gray-600">{{ __('general.Your wishlist is empty.') }}</p>
                    @endif
                </div>
                
                <!-- Address Summary -->
                <div class="bg-white p-6 rounded-lg shadow-md">
                    <h3 class="text-lg font-semibold mb-2">{{ __('general.Saved Addresses') }}</h3>
                    @if($addresses->count() > 0)
                        <p class="text-gray-600">{{ __('general.You have :count saved addresses.', ['count' => $addresses->count()]) }}</p>
                        <a href="{{ route('buyer.dashboard.addresses') }}" class="text-blue-500 hover:text-blue-700 mt-2 inline-block">{{ __('general.Manage addresses') }}</a>
                    @else
                        <p class="text-gray-600">{{ __('general.You have no saved addresses.') }}</p>
                    @endif
                </div>
            </div>
